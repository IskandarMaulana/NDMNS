...
namespace NDMNS.Controllers
{
    public class IspController : BaseController
    {
        ...
        public IspController(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            try
            {
                ...
                {
                    TempData["Error"] = "Failed to load ISP data";
                    return View(new List<Isp>());
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Create(Isp isp)
        {
            var userId = HttpContext.Session.GetString("Id");
            if (string.IsNullOrEmpty(userId))
            {
            ...
            try
            {
                var jsonContent = JsonSerializer.Serialize(isp);
                ...
                {
                    var errorContent = await response.Content.ReadFromJsonAsync<DtoResponse<Isp>>();
        ...
        public async Task<IActionResult> Update(string id)
        {
            ...
            try
            {
                var apiUrl = $"https://localhost:44318/api/Isp/{id}";
                var response = await _httpClient.GetAsync(apiUrl);

                if (response.IsSuccessStatusCode)
                {
                    var responseContent = await response.Content.ReadAsStringAsync();
                    var apiResponse = JsonSerializer.Deserialize<DtoResponse<Isp>>(
                        responseContent,
                        new JsonSerializerOptions
                        {
                            PropertyNamingPolicy = JsonNamingPolicy.CamelCase,
                ...
                TempData["Error"] = "ISP not found";
                return RedirectToAction("Index");
            }
            catch (Exception ex)
            {
                TempData["Error"] = "Error loading ISP data\n" + ex.Message;
                return RedirectToAction("Index");
            }
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Update(string id, Isp isp)
        {
            var userId = HttpContext.Session.GetString("Id");
            if (string.IsNullOrEmpty(userId))
            {
            ...
            if (id != isp.Id)
            {
                TempData["Error"] = "Invalid ISP data";
                return RedirectToAction("Index");
            }
